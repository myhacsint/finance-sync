...
window.FinanceMonthCheck = (() => {
  ...
  function tabs(isCheck = active()) {
    return '<nav class="month-check-tabs" aria-label="Prüfbereich"><a data-mc-tab="bookings" href="'+href('bookings')+'"'+(!isCheck?' aria-current="page"':'')+'>Buchungen</a><a data-mc-tab="month-check" href="'+href('month-check',month())+'"'+(isCheck?' aria-current="page"':'')+'>Monatscheck</a></nav>';
  }
