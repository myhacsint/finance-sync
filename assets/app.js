...
function navMarkup(compact=false){
  ...
  const extra=navItems.filter(item=>!primary.includes(item.view));
  const activeExtra=extra.find(item=>item.view===navCurrent);
  return navItems.filter(item=>primary.includes(item.view)).map(link).join("")+'<details class="mobile-more"><summary class="nav-item'+(activeExtra?' is-current':'')+'" aria-label="Weitere Bereiche'+(activeExtra?', aktuell: '+esc(activeExtra.label):'')+'">'+icons.chevron+'<span>Mehr</span></summary><div class="mobile-more-menu">'+extra.map(link).join("")+'</div></details>';
}
...
let token=sessionStorage.getItem("financeToken")||localStorage.getItem("financeToken")||"";
localStorage.removeItem("financeToken");
sessionStorage.removeItem("financeToken");
// Retire old URLs without retaining free-text searches in browser history.
if(new URLSearchParams(location.search).has("expenseSearch")){
  const url=new URL(location.href);url.searchParams.delete("expenseSearch");history.replaceState(null,"",url);
}
let sessionExchange;
...
async function apiRequest(path,options={},retry=true){
  if(token){
    ...
    if(!response.ok){requestToken();throw new Error("Bitte Zugang erneut bestätigen.");}
  }
...
function toggleConfirmation(element,id){document.getElementById(id).disabled=!element.checked;}
function toggleSutorConfirmation(element){document.getElementById("sutor-confirm-button").disabled=!element.checked||!currentSutorPreview?.canConfirm;}
function setNewsletterSource(value){setNewsletterFilters({source:value});}
function setNewsletterState(value){setNewsletterFilters({state:value});}
function activateAnalysisPosition(event,key){if(event.key==="Enter"||event.key===" "){event.preventDefault();toggleAnalysisPosition(key);}}
registerUiActions({headerAction,activateAnalysisPosition,applyAnalysisFilters,applyDecisionLab,applyFireScenario,applyNamedScenario,applyRecurringFilters,closeReviewMonth,compareNamedScenarios,confirmManual,confirmPension,confirmSutor,continueDkb,deleteLifeEvent,deleteMerchantRule,discardPension,discardSutor,exportNow,filterExpenseCategories,importMilesMore,openManual,pensionFileSelected,preflightDkb,previewManual,previewMilesMore,previewPensionFire,reconcile,refresh,resetMilesMorePreview,saveLifeEvent,saveMerchantRule,saveNamedScenario,saveRecurringDecision,saveRecurringOptimization,saveReviewTransaction,setAnalysisView,setAssetArea,setCashflowMonths,setExpenseAccount,setExpenseCategory,setExpensePage,setExpensePeriod,setFireCategoryDetailPeriod,setLabView,setNewsletterGroupState,setNewsletterSource,setNewsletterState,setReviewMonths,setSpendingOffset,setWealthHistoryRange,shiftCashflow,shiftSpending,sutorFileSelected,syncSource,toggleAnalysisCategories,toggleAnalysisPosition,toggleAssetHoldings,toggleConfirmation,toggleExpenseCategories,toggleExpenseSort,toggleFireCategoryDetail,toggleNewsletter,toggleRecurringCandidate,toggleSutorConfirmation,updateExpenseSearch,uploadPension,uploadSutor});
...
function setExpenseParams(changes,replace=false){
  ...
  Object.entries(changes).forEach(([key,value])=>{
    if(key==="search"){expenseSearchValue=String(value??"").trim().slice(0,80);params.delete("expenseSearch");return;}
...
function expenseSortHeader(column,label,sort){
  ...
  return '<th><button type="button" class="expense-sort'+(active?" is-active":"")+'" data-fh-click="toggleExpenseSort(&quot;'+column+'&quot;)" aria-label="'+esc(label)+' '+dir+' sortieren">'+esc(label)+(active?(current.endsWith("-asc")?" ↑":" ↓"):"")+"</button></th>";
}
...
function csvCell(value){
  ...
  if(typeof value==="string"&&/^[\s\uFEFF]*[=+@-]/.test(text)&&!/^-[0-9]+(?:,[0-9]+)?$/.test(text))text="'"+text;
...
function renderOverview(data,history){
  ...
  const comparisonSummary=Number.isFinite(comparison.changeTotalMinor)
    ?'<div class="wealth-change-summary"><span>Seit '+esc(formatDate(comparison.effectiveDate))+'</span><strong class="wealth-change-'+totalChangeTone+'">'+signedMoneyWhole(comparison.changeTotalMinor)+'</strong><small>Vergleichbare historische Bewertung [SCHÄTZUNG]</small></div>'
  ...
  const historyPanel='<details class="wealth-history" open><summary><span><strong>Vermögensverlauf</strong><small>Historie aus Actual, Ghostfolio und FinanceSync</small></span>'+icons.chevron+'</summary><div class="wealth-history-controls" role="group" aria-label="Zeitraum für Vermögensverlauf"><button type="button" data-wealth-range="1m" aria-pressed="false" data-fh-click="setWealthHistoryRange(\'1m\')">1 M.</button><button type="button" data-wealth-range="ytd" aria-pressed="false" data-fh-click="setWealthHistoryRange(\'ytd\')">YTD</button><button type="button" data-wealth-range="1y" aria-pressed="true" data-fh-click="setWealthHistoryRange(\'1y\')">1 J.</button><button type="button" data-wealth-range="max" aria-pressed="false" data-fh-click="setWealthHistoryRange(\'max\')">Max</button></div><div id="wealth-history-content" class="wealth-history-content" aria-live="polite"></div></details>';
  ...
  const cashflow=data.cashflow.state==="current"?'\
    ...
    <div class="sr-only"><table><caption>Geldfluss: '+esc(rangeAccessible)+'</caption><thead><tr><th>Monat</th><th>Einnahmen</th><th>Ausgaben</th></tr></thead><tbody>'+chartTable+'</tbody></table></div>'
  ...
  const spendingControls=data.spending.state==="current"?'\
    <div class="period-controls" aria-label="Monat für Ausgaben"><button class="range-previous" type="button" data-fh-click="shiftSpending(1)" aria-label="Einen Ausgabenmonat zurück" title="Einen Monat zurück">'+icons.chevron+'</button><label class="sr-only" for="spending-month">Angezeigter Ausgabenmonat</label><select class="spending-window" id="spending-month" name="spending-month" autocomplete="off" data-fh-change="setSpendingOffset(this.value)">'+spendingMonthOptions(latestSpendingMonth,selectedSpendingOffset)+'</select><button class="range-next" type="button" data-fh-click="shiftSpending(-1)" aria-label="Einen Ausgabenmonat vor" title="Einen Monat vor"'+(selectedSpendingOffset===0?' disabled':'')+'>'+icons.chevron+'</button></div>':'';
  ...
  document.getElementById("dashboard").innerHTML='\
    ...
    <div class="overview-dashboard-grid">\
      <section class="overview-panel" data-month-count="'+months.length+'" aria-labelledby="cashflow-title"><div class="panel-header cashflow-panel-header"><div><h2 id="cashflow-title">Geldfluss</h2><p class="cashflow-period">'+esc(rangeLabel)+' <span aria-hidden="true">·</span> '+esc(rangeDetail)+'</p></div><div class="period-controls" aria-label="Zeitraum für Geldfluss"><button class="range-previous" type="button" data-fh-click="shiftCashflow(1)" aria-label="Einen Monat zurück" title="Einen Monat zurück">'+icons.chevron+'</button><label class="sr-only" for="cashflow-window">Angezeigter Zeitraum</label><select class="cashflow-window" id="cashflow-window" name="cashflow-window" autocomplete="off" data-fh-change="setCashflowMonths(this.value)"><option value="4"'+(range.months===4?' selected':'')+'>4 Monate</option><option value="6"'+(range.months===6?' selected':'')+'>6 Monate</option><option value="12"'+(range.months===12?' selected':'')+'>12 Monate</option></select><button class="range-next" type="button" data-fh-click="shiftCashflow(-1)" aria-label="Einen Monat vor" title="Einen Monat vor"'+(range.offset===0?' disabled':'')+'>'+icons.chevron+'</button></div></div>'+cashflow+'</section>\
...
function expenseState(title,text,action,label,tone="critical"){
  return '<section class="expense-state" role="status"><div class="expense-state-inner">'+statusIcon(tone)+'<h2>'+esc(title)+'</h2><p>'+esc(text)+'</p>'+(action?'<button class="button secondary" type="button" data-fh-click="'+action+'">'+esc(label)+'</button>':'')+'</div></section>';
}
...
function renderSpending(data){
  ...
  const monthControls='<div class="period-controls expense-period-controls" aria-label="Ausgabenzeitraum"><div class="lab-nav" role="tablist">'+[["month","Monat"],["quarter","Quartal"],["ytd","YTD"],["year","Jahr"]].map(([key,label])=>'<button type="button" class="lab-nav-item" aria-current="'+String(periodKind===key)+'" data-fh-click="setExpensePeriod(&quot;'+key+'&quot;,'+(key==="month"?"&quot;"+esc(period.endMonth||data.month)+"&quot;":key==="quarter"?"&quot;"+(period.key.includes("Q")?period.key:period.endMonth.slice(0,4)+"-Q"+Math.ceil(Number((period.endMonth||data.month).slice(5))/3))+"&quot;":"&quot;"+(period.endMonth||data.month).slice(0,4)+"&quot;")+')">'+label+"</button>").join("")+'</div>'+(periodKind==="month"?'<label class="sr-only" for="expense-month">Monat</label><select id="expense-month" autocomplete="off" data-fh-change="setExpensePeriod(&quot;month&quot;,this.value)">'+expenseMonthOptions(newestMonth,data.oldestMonth,period.endMonth||data.month)+"</select>":periodKind==="quarter"?'<label class="sr-only" for="expense-quarter">Quartal</label><select id="expense-quarter" autocomplete="off" data-fh-change="setExpensePeriod(&quot;quarter&quot;,this.value)">'+expenseQuarterOptions(newestMonth,data.oldestMonth,period.key)+"</select>":'<label class="sr-only" for="expense-year">Jahr</label><select id="expense-year" autocomplete="off" data-fh-change="setExpensePeriod(&quot;'+periodKind+'&quot;,this.value)">'+expenseYearOptions(newestMonth,data.oldestMonth,(period.endMonth||data.month).slice(0,4))+"</select>")+(period.complete?"":'<span class="analysis-estimate">laufender Monat</span>')+"</div>";
...
function renderAnalyses(data){
  ...
  const toolbar='<section class="analysis-toolbar" aria-label="Analysefilter"><label>Ansicht<select name="analysis-view" data-fh-change="setAnalysisView(this.value)"><option value="expense-structure" selected>Ausgabenstruktur</option><option value="investment-newsletters">Investment-Cockpit</option><option value="crypto-origin-tax">Krypto · Herkunft &amp; Steuerstatus</option></select></label><label>Zeitraum<select id="analysis-period" name="analysis-period" autocomplete="off">'+periodOptions+'</select></label><label>Vergleich<select id="analysis-comparison" name="analysis-comparison" autocomplete="off">'+comparisonOptions+'</select></label><button class="button" type="button" data-fh-click="applyAnalysisFilters()">Anwenden</button></section>';
  ...
  const categoryMore=data.categories.length>8?'<button class="analysis-more" type="button" data-fh-click="toggleAnalysisCategories()">'+(state.expanded?'Weniger Kategorien':'Alle '+data.categories.length+' Kategorien anzeigen')+'</button>':'';
  ...
  const desktopRows=positions.map(row=>{
    ...
    return '<tr class="analysis-position-row" tabindex="0" role="button" aria-expanded="'+String(open)+'" data-fh-click="toggleAnalysisPosition(&quot;'+esc(row.key)+'&quot;)" data-fh-keydown="activateAnalysisPosition(event,&quot;'+esc(row.key)+'&quot;)"><td>'+esc(row.label)+' '+analysisEstimate(row.estimate)+'</td><td>'+esc(row.category)+'</td><td>'+esc(row.class)+'</td><td>'+moneyWhole(row.amountMinor)+'</td><td>'+icons.chevron+'</td></tr><tr class="analysis-position-detail"'+(open?'':' hidden')+'><td colspan="5"><div class="analysis-months">'+months+'</div></td></tr>';
  }).join("");
...
function renderCryptoAnalysis(data){
  ...
  const toolbar='<section class="analysis-toolbar crypto-toolbar" aria-label="Auswahl und Datenstand der Kryptoanalyse"><label>Ansicht<select name="analysis-view" autocomplete="off" data-fh-change="setAnalysisView(this.value)"><option value="expense-structure">Ausgabenstruktur</option><option value="investment-newsletters">Investment-Cockpit</option><option value="crypto-origin-tax" selected>Krypto · Herkunft &amp; Steuerstatus</option></select></label><div class="crypto-toolbar-meta"><span>Prüfumfang</span><strong>Ab '+data.selection.scopeStartYear+'</strong></div><div class="crypto-toolbar-meta"><span>Rekonstruktionsstand</span><strong>'+esc(formatDate(data.capturedAt,true))+'</strong></div></section>';
...
function renderRecurringOptimizations(data){
  ...
  const toolbar='<section class="analysis-toolbar optimization-toolbar" aria-label="Optimierungsliste"><label>Ansicht<select name="analysis-view" autocomplete="off" data-fh-change="setAnalysisView(this.value)"><option value="expense-structure">Ausgabenstruktur</option><option value="crypto-origin-tax">Krypto · Herkunft &amp; Steuerstatus</option></select></label></section>';
...
function renderReview(data){
  ...
  const windowBar='<section class="analysis-toolbar" aria-label="Prüffenster"><label>Zeitraum<select id="review-months" data-fh-change="setReviewMonths(this.value)"><option value="3"'+(months===3?' selected':'')+'>3 Monate</option><option value="6"'+(months===6?' selected':'')+'>6 Monate</option><option value="12"'+(months===12?' selected':'')+'>12 Monate</option><option value="24"'+(months===24?' selected':'')+'>24 Monate</option></select><small>'+esc(data.window?.startDate||'')+'–'+esc(data.window?.endDate||'')+'</small></label></section>';
  ...
  const bookingRow=(line,income)=>{
    ...
    return '<article class="review-booking"><div><strong>'+esc(line.merchant)+'</strong><span>'+esc(formatDate(line.date))+' · '+esc(line.accountLabel)+(line.notes?' · '+esc(line.notes):'')+'</span></div><strong>'+(income?'+':'')+money(line.amountMinor)+'</strong><label>Actual-Kategorie<select id="review-category-'+fieldId+'" autocomplete="off">'+reviewCategoryOptions(cats,line.categoryKey==="uncategorized"?"":line.categoryKey)+'</select></label><label>Händler bündeln als<input id="review-alias-'+fieldId+'" value="'+esc(line.merchant)+'" maxlength="80"></label><button class="button" id="review-save-'+fieldId+'" type="button" data-fh-click="saveReviewTransaction(&quot;'+esc(line.id)+'&quot;)">In Actual speichern</button></article>';
  };
  const currentMonth=data.window?.currentMonth||"";
  const partition=(rows,current)=>(rows||[]).filter(line=>String(line.date||"").startsWith(currentMonth)===current);
  const periodRows=(rows,income,current)=>{
    const matching=partition(rows,current);
    const label=current?'Laufender Monat · '+analysisMonthLabel(currentMonth):'Abgeschlossene Monate · '+months+' Monate';
    const note=current?'Bis '+formatDate(data.window?.currentEndDate||data.generatedAt)+' · unvollständig':esc(data.window?.startDate||'')+'–'+esc(data.window?.endDate||'');
    return '<section class="review-period-group" aria-label="'+esc(label)+'"><div class="review-period-head"><div><strong>'+esc(label)+'</strong><small>'+note+'</small></div><b>'+matching.length+'</b></div><div class="review-booking-list">'+(matching.map(line=>bookingRow(line,income)).join("")||'<div class="empty">Keine offenen Buchungen.</div>')+'</div></section>';
  };
  const expenseRows=periodRows(data.uncategorizedExpenses||data.uncategorized||[],false,true)+periodRows(data.uncategorizedExpenses||data.uncategorized||[],false,false);
  const incomeRows=periodRows(data.uncategorizedIncome||[],true,true)+periodRows(data.uncategorizedIncome||[],true,false);
...
function renderDecisionLab(data){
  ...
  const scenarioBar='<details class="management scenario-management"><summary>Szenarien und Ereignisse verwalten</summary><div class="management-body"><section class="analysis-toolbar" aria-label="Gespeicherte Szenarien"><label>Szenario<select id="named-scenario" data-fh-change="applyNamedScenario(this.value)"><option value="">Aktuelles Labor</option></select></label><label>Name<input id="scenario-name" maxlength="80" placeholder="z. B. Krippe endet"></label><button class="button secondary" type="button" data-fh-click="saveNamedScenario()">Szenario speichern</button></section><section class="analysis-toolbar" aria-label="Ereignisse"><label>Ereignis<input id="event-name" placeholder="Krippe endet"></label><label>Ab<input id="event-start" type="month"></label><label>€ / Monat<input id="event-amount" type="number" step="1"></label><button class="button secondary" type="button" data-fh-click="saveLifeEvent()">Ereignis speichern</button></section><div id="life-event-list" class="compact-manage-list"><p class="empty">Ereignisse werden geladen …</p></div><section class="analysis-toolbar" aria-label="Szenarien vergleichen"><label>Ausgang<select id="compare-left" aria-label="Ausgangsszenario"></select></label><label>Vergleich<select id="compare-right" aria-label="Vergleichsszenario"></select></label><button class="button secondary" type="button" data-fh-click="compareNamedScenarios()">Vergleichen</button><div id="scenario-compare"></div></section></div></details>';
  const toolbar='<nav class="lab-nav" aria-label="Laborbereiche"><button class="button '+(currentLab==="fire"?'':'secondary')+'" type="button" data-fh-click="setLabView(&quot;fire&quot;)">FIRE-Kurs</button><button class="button '+(currentLab==="year"?'':'secondary')+'" type="button" data-fh-click="setLabView(&quot;year&quot;)">Jahresausblick</button><button class="button '+(currentLab==="path"?'':'secondary')+'" type="button" data-fh-click="setLabView(&quot;path&quot;)">Trajektorie</button></nav>';
...
function renderTask(task){
  const action=task.id==="sutor-riester"
    ? '<a class="button secondary" href="#/sutor-riester">PDF aktualisieren</a>'
    : '<button class="button secondary" type="button" data-fh-click="openManual(&quot;'+encoded(task.id)+'&quot;)">Werte aktualisieren</button>';
  return '<article class="task-card"><span class="task-mark">'+icons.manual+'</span><div><h3>'+esc(task.label)+'</h3><p>Letzter bestätigter Wert: '+esc(formatDate(task.valueDate))+'</p></div>'+action+'</article>';
}
...
const sutorValidationLabels={
  OCR_REVIEW_REQUIRED:"Die Tabelle wurde per OCR erkannt und kann nicht automatisch bestätigt werden. Bitte den digitalen, maschinenlesbaren PDF-Auszug verwenden.",
...
function renderSutorHome(revisions=[]){
  ...
  document.getElementById("dashboard").innerHTML=pensionStep(1)+'<section class="pension-security-note" role="note">'+icons.check+'<div><strong>Privat verarbeitet</strong><span>PDF und Volltext werden nach der lokalen Erkennung gelöscht. Gespeichert werden nur bestätigte strukturierte Werte.</span></div></section><section class="pension-upload-panel" aria-labelledby="sutor-upload-title"><div class="pension-upload-copy"><span class="pension-document-icon">'+icons.manual+'</span><div><h2 id="sutor-upload-title">Monatlichen Sutor-Depotauszug prüfen</h2><p>PDF · maximal 12 MB · maximal 12 Seiten</p></div></div><form id="sutor-upload-form" data-fh-submit="uploadSutor(event)"><label class="pension-file-control" for="sutor-document"><span>PDF auswählen</span><input id="sutor-document" name="document" type="file" accept="application/pdf" required data-fh-change="sutorFileSelected(this)"></label><span id="sutor-selected-file" class="pension-selected-file">Noch keine PDF ausgewählt</span><button class="button" type="submit">Sicher prüfen</button></form><ul class="pension-privacy-list"><li>Name, Adresse, Depotnummer, IBAN und Umsatzseiten gelangen nicht in die Oberfläche.</li><li>Positionen werden dynamisch über gültige ISIN erkannt; neue ISIN werden niemals still zugeordnet.</li><li>Der bestätigte Sutor-Stand ist Referenz und Zeitreihe. Der aktuelle Vermögenswert kommt aus Ghostfolio; beide Werte werden nicht addiert.</li></ul></section><section class="pension-history" aria-labelledby="sutor-history-title"><div class="section-heading"><div><h2 id="sutor-history-title">Bestätigte PDF-Stände</h2><p>Neue Sutor-Stände werden ausschließlich über den sicheren PDF-Import übernommen.</p></div></div>'+sutorHistory(revisions)+'</section>';
  document.getElementById("dashboard").setAttribute("aria-busy","false");
}
...
function renderDashboard(data){
  ...
  document.getElementById("dashboard").innerHTML='\
    ...
    <section class="section"><details class="management" id="management"><summary>Verwaltung und manuelle Eingabe</summary><div class="management-body"><div class="management-tools"><button class="button secondary" type="button" data-fh-click="exportNow()">CSV neu erzeugen</button><button class="button secondary" type="button" data-fh-click="reconcile()">Interne Überträge abgleichen</button></div><section class="manual-workflow" id="miles-section"><h2>Miles &amp; More Abrechnung</h2><p>Text der Kreditkartenabrechnung einfügen. Vorschau ändert noch nichts.</p><div class="manual-grid"><div><label for="miles-date">Abrechnungsdatum</label><input id="miles-date" type="date" data-fh-input="resetMilesMorePreview()"></div><div><label for="miles-text">Abrechnungstext</label><textarea id="miles-text" data-fh-input="resetMilesMorePreview()" placeholder="Hier die Umsätze inkl. Saldo einfügen …"></textarea></div></div><div class="actions" style="margin-top:12px"><button class="button" type="button" data-fh-click="previewMilesMore()">Vorschau prüfen</button><button class="button secondary" id="miles-import-button" type="button" data-fh-click="importMilesMore()" disabled>In Actual importieren</button></div><div id="miles-preview" class="preview"></div></section><section class="manual-workflow" id="manual-section" hidden><h2>Manuelle Vorsorge-Eingabe</h2><p>Diese Texteingabe ist ausschließlich für weiterhin manuell geführte Vorsorgeverträge vorgesehen.</p><div class="manual-grid"><div><label for="manual-source">Vertrag</label><select id="manual-source" name="manual-source" autocomplete="off"></select></div><div><label for="manual-text">Kopierter Text</label><textarea id="manual-text" name="manual-text" autocomplete="off" spellcheck="false" placeholder="Hier den vollständigen Text einfügen …"></textarea></div></div><div class="actions" style="margin-top:12px"><button class="button" id="preview-button" type="button" data-fh-click="previewManual()">Vorschau prüfen</button><span style="color:var(--muted)">Noch kein Import</span></div><div id="manual-error" class="notice error" role="status" aria-live="polite"></div><div id="manual-preview" class="preview"></div></section></div></details></section>';
  document.getElementById("dashboard").setAttribute("aria-busy","false");
}
...
async function refresh(force=false){
  ...
  const call=async(path,options={})=>{const result=await apiRequest(path,{...options,signal});if(generation!==refreshGeneration)throw new DOMException("Veraltete Anfrage","AbortError");return result;};
  ...
  try{
    if(view==="overview"){
      ...
      const [data,history]=await Promise.all([call("/api/dashboard/overview?"+params.toString()),call("/api/dashboard/wealth-history"+historyParams).catch(error=>{if(error.name==="AbortError")throw error;return null;})]);renderOverview(data,history);
