...
import { assertSafePdfJson, runBounded, scanMalware, type CommandRunner } from "./pension-security.js";
...
function parseTsv(tsv: string): { text: string; confidence: number } {
...
async function extractPdf(path: string, workDir: string, runner: CommandRunner): Promise<ExtractedPensionDocument> {
  await runner("/usr/bin/qpdf", ["--check", path], 20_000);
  const qpdfJson = await runner("/usr/bin/qpdf", ["--json", path], 20_000);
  assertSafePdfJson(qpdfJson.stdout);
  const info = await runner("/usr/bin/pdfinfo", [path], 20_000);
  ...
  if (pageCount > 6) throw new Error("PDF_TOO_MANY_PAGES");

  const nativePages: PensionTextPage[] = [];
  for (let page = 1; page <= pageCount; page += 1) {
    const output = join(workDir, `native-${page}.txt`);
    await runner("/usr/bin/pdftotext", ["-f", String(page), "-l", String(page), "-layout", "-nopgbrk", path, output], 20_000);
  ...
  await runner("/usr/bin/pdftoppm", ["-png", "-r", "200", path, prefix], 60_000);
