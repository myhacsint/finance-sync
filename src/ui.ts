...
function renderDecisionLab(data){
  ...
  const annualPanel=annual.available?'<section class="analysis-panel decision-annual" aria-labelledby="decision-annual-title"><div class="analysis-panel-head"><div><h2 id="decision-annual-title">Jahresausblick '+esc(annual.year)+'</h2><p>'+esc(annual.completedMonths)+' abgeschlossene Monate bis '+esc(analysisMonthLabel(annual.throughMonth))+' · laufender Monat nicht eingerechnet.</p></div></div><div class="decision-annual-progress" aria-label="'+esc(annual.completedMonths)+' von 12 Monaten abgeschlossen">'+annualMonths+'</div><div class="decision-context-grid" style="margin-top:14px"><table class="decision-context-table decision-annual-table"><thead><tr><th scope="col">Jahressicht</th><th scope="col">Einnahmen</th><th scope="col">Ausgaben</th><th scope="col">Saldo</th></tr></thead><tbody>'+annualRow('Ist bis '+analysisMonthLabel(annual.throughMonth),annual.actualToDate,false)+annualRow('Median-Pfad bis '+analysisMonthLabel(annual.throughMonth)+' '+analysisEstimate(true),annual.expectedToDate,false)+annualRow('Hochrechnung Jahresende '+analysisEstimate(true),annual.projectedYearEnd,true)+annualRow('Median × 12 · Referenz '+analysisEstimate(true),annual.medianFullYear,false)+'</tbody></table></div><div class="decision-annual-variance"><span>Ist-Saldo gegenüber erwartetem Pfad</span><strong class="'+(annualVariance<0?'tone-warning':'tone-ok')+'">'+esc(annualVarianceText)+'</strong></div><p class="decision-breakdown-note">Die Jahresend-Hochrechnung verbindet den echten Stand der abgeschlossenen Monate mit dem typischen Median-Monat für die verbleibenden '+esc(annual.remainingMonths)+' Monate. Der laufende Monat bleibt bis zum Abschluss separat.</p></section>':'';
